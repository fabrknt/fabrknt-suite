...
import { IndexLinkSchema, type IndexLinkInput, type CreateListingInput } from '@/lib/schemas/listing';
...
export function IndexLinkStep({ defaultValues, onSubmit, onPrevious }: IndexLinkStepProps) {
  ...
  const handleSearch = async () => {
    ...
    setIsSearching(true);
    try {
      const response = await fetch(`/api/index/search?q=${encodeURIComponent(searchQuery)}`);
      if (response.ok) {
  ...
  const handleSelectCompany = async (company: any) => {
    ...
    try {
      const response = await fetch(`/api/index/${company.id}`);
      if (response.ok) {
  ...
  return (
    <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold mb-2">Link Index Data (Optional)</h2>
        <p className="text-muted-foreground">
          Boost credibility by linking your listing to Fabrknt Index data (PULSE + TRACE)
        </p>
      </div>

      {/* Benefits */}
      <div className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-lg p-4">
        <h3 className="font-semibold text-purple-900 mb-3">Why link Index data?</h3>
      ...
      {errors.indexCompanyId && (
        <p className="text-sm text-red-600">{errors.indexCompanyId.message}</p>
      )}
