/**
 * @fabrknt/accredit-core integration — On-chain KYC/AML verification.
 *
 * Thin adapter layer between Forge's domain types and the real @fabrknt/accredit-core SDK.
 * - Uses @fabrknt/accredit-core KycLevel enum for level ordering
 * - Uses @fabrknt/accredit-core KYC_TRADE_LIMITS for trade limit lookups
 * - WrapperConfig/WrapRequest/WrapResult types come from @fabrknt/accredit-core
...
export async function verifyIdentity(
  ...
  // In production: queries @fabrknt/accredit-core on-chain program via KycClient
...
// Uses @fabrknt/accredit-core KycLevel enum for ordering
// ---------------------------------------------------------------------------

/**
 * Map Forge KycLevel strings to @fabrknt/accredit-core KycLevel enum values for ordering.
 */
...
export async function isBlacklisted(walletAddress: string): Promise<boolean> {
  // In production: queries @fabrknt/accredit-core BlacklistClient.findBlacklistEntryPda()
  return false;
}
...
// ---------------------------------------------------------------------------
// Compliant token wrapping (uses @fabrknt/accredit-core wrapper types)
// ---------------------------------------------------------------------------
...
export async function wrapTokens(
  ...
  // In production: calls @fabrknt/accredit-core WrapperClient.wrap()
...
export async function unwrapTokens(
  ...
  // In production: calls @fabrknt/accredit-core WrapperClient.unwrap()
  return {
    success: false,
    wrappedAmount: BigInt(0),
    fee: BigInt(0),
